...
fun SignUpScreen(viewModel: AuthViewModel, onNavigateToSignIn: () -> Unit) {
    ...
    Column(
        ...
        Button(onClick = viewModel::signUp, enabled = !uiState.isLoading, modifier = Modifier.fillMaxWidth()) {
            if (uiState.isLoading) CircularProgressIndicator(modifier = Modifier.size(24.dp)) else Text("Sign Up")
        }
